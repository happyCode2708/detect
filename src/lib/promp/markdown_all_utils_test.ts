...
export const make_markdown_all_prompt_test = ({
  ocrText,
  imageCount,
}: {
  ocrText?: string;
...
11) Each table provide different list item. Only process given items in its own table. Do not interchange item/claim between tables.
...
2) CHECK_CLAIM_TABLE rules:
+ all row of prior table is input rows of CHECK_CLAIM_TABLE
...
FIRST_EXTRA_CLAIM_TABLE
| text| is exact text explicitly visibly seen on product images ? (answer is yes/no/unknown) ? | does product explicitly  say contain it ? (answer is yes/no/unknown) | how do you know ? | do you know it through which info ? (answer are "ingredient list"/ "nutrition fact"/ "marketing text on product"/ "others") (answer could be multiple string from many sources) (answer could be multiple string from many sources)| how do you know that ? and give me you explain (answer in string) | do you know it through which info ? (answer are "ingredient list"/ "nutrition fact"/ "marketing text on product"/ "others") |
| ------- | -------- | -------- |
+ additives:
| additives | ...
...
3) third extra claim list info recorded in markdown table format below 
...
4) third extra claim list info recorded in markdown table format below 
...
5) Sugar claim info recorded in markdown table format below 
...
6) Fat claim info of product images recorded in markdown table format below 
...
7) other claim list info recorded in markdown table format below 
...
8) calorie claim info recorded in markdown table format below:
(ROW RETURN CONDITION: only return row item if "does product explicitly claim this claim" value = true )
...
9) salt claim info recorded in markdown table format below:
(ROW RETURN CONDITION: only return row item if "does product explicitly claim this claim" value = true )
...
13) Physical info with table format below

PHYSICAL_TABLE
| upc-12 or gtin-12 |
| ------- |

14) Marketing info with table format below:
...
15) Instruction info with table format below:
...
16) supply chain info with table format below:
