...
Some rules for you:
1)  this fact panel could be in the dual-column layout showing both "per serving" and "per container" information. Let's break down and separate into two different fact panels one is for 'per serving'
 and other for 'per container'
...
const hint = `Nutrition labels can be confusing, especially with the dual-column layout showing both "per serving" and "per container" information. Let's break down and separate into two different fact panel`;

const test_prompt = `
Carefully examine the image provided and created a neatly formatted JSON output containing a list of objects only if info available on image and Each object should contain:
json
[
{
"panelName": string ,
